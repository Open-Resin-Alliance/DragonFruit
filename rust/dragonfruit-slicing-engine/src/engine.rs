...
///
/// NOT the only RAM arbiter in the app, and deliberately so — do not "unify" the two.
/// This gate is an *in-flight throttle*: it bounds native slicer RAM, continuously,
/// during a slice job, adapting to machine state at dispatch time (non-deterministic
/// by construction). The other arbiter, `src-tauri/src/stl_budget.rs`, is an
/// *admission gate*: it bounds the WebView renderer's JS heap, once, before an import,
/// as a pure deterministic function of (heap limit, system RAM, model size). Different
/// resource, different process, different phase, different quantity; neither reads the
/// other's output.
struct GateState {
    count: usize,
    max: usize,
