...
**Correction (2026-08-27).** After v0.1.14 shipped, Linux users reported a blank build plate and attributed it to this decision — reasonably, since this is the only Linux-facing change in the release and it is the document they found. That attribution is wrong, and this ADR helped spread it. The blank plate is #606: `tauri.conf.json` set `"transparent": true` on the splash screen, and under CEF a transparent window crashes the GPU process (`exit_code=139`), after which *every* window in the process loses its WebGL context — including the main window, which is not transparent. Upstream tauri-apps/tauri#15718. Setting `transparent: false` restores rendering at full frame rate with the X11 grant unchanged. Nothing below about windowing is retracted; what is retracted is the implication that the consequences listed here are the *only* things that changed for Linux users in that release.
