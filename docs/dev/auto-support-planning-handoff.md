...
- Generation ends with empirical verification (`src/supports/autoSupport/verifyCoverage.ts`): the model is re-scanned merged with the planned and committed support geometry, one bounded repair round routes supports for whatever the scan still sees (tight spacing floor, doubled surface search), and a second scan confirms. Support contact points get small weld spheres in the verification geometry — printed tips penetrate and fuse, and without the weld a sub-pixel contact fails the scan's minimum layer overlap. Flagged spots with a fused tip within 3 mm are scan artifacts (voxel linkage, self-intersecting unioned source meshes — one confirmed enclosed internal void on the test model) and are excluded from the reported count.
