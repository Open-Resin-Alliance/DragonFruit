...
- **Anti-topple coverage is gated on the verdict, not on the classification.** A steep flat is worth contact only if the pose can actually topple, so `needsToppleCoverage` (the constant-free half plus the conservative adhesion bound, in `poseStability.ts`) is asked by both consumers: `computeStabilizationAnchors` decides whether to lay anchors at all, and `computeAutoSupportPlan` drops `steepFlat` islands from the candidate set when it says no. Measured on a cam seal tool: `bearing 1755.7mm² over 85 edges`, `centroid depth 23.63mm`, `adhesion 0.790` against a conservative `p/σ` of 0.05. The part was wrapped in a support forest anyway, because the steep-flat coverage never asked. It logs why when it skips: `Topple coverage not needed — 11 steep flats left uncovered (adhesion 0.790, centroid depth 23.63mm, bearing 1755.7mm²)`.
