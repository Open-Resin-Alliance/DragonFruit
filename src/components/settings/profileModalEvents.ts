...
export type OpenProfileSettingsModalOptions = {
  ...
  openMaterialAntiAliasing?: boolean;
};

export function openProfileSettingsModal(tab: ProfileSettingsTab, options?: OpenProfileSettingsModalOptions): void {
  if (typeof window === 'undefined') return;
  window.dispatchEvent(new CustomEvent<{ tab: ProfileSettingsTab; openPrinterLibrary?: boolean; openNetworkSettings?: boolean; openMaterialAntiAliasing?: boolean }>(OPEN_PROFILE_SETTINGS_MODAL_EVENT, {
    detail: {
      tab,
      openPrinterLibrary: options?.openPrinterLibrary === true,
      openNetworkSettings: options?.openNetworkSettings === true,
      openMaterialAntiAliasing: options?.openMaterialAntiAliasing === true,
    },
  }));
}
