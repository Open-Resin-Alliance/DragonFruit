...
export default function RaftRenderer() {
  ...
  const meshes = React.useMemo(() => {
    if (!raft.enabled) return null;

    // Build circles from supports' base positions and diameters
  ...
  }, [supports, raft.enabled, raft.thickness, raft.chamferAngle, raft.wallHeight, raft.wallThickness]);
