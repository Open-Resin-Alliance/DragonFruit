...
[X] curved supports?
[ ] Selection groupings for supports. Select by Tip size + or -. Set grouping tags.
    You could have a sorta "selection mode"  where you click a support the "show matching" or something where it would select all matching supports based on tip size. 
    It could be "select" or "show only" or "hide"
    This could be a hotkey or a tool button that opens a submenu. 
    It could also be a right click to open a submenu that has these options.
    Lychee has something like this, but its just a broad selection menu. 
