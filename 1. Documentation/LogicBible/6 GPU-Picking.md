...
- **Update rate**: 30 times per second during normal hover; 60 times per second during drags.
- **When it updates**: While the pointer is inside the canvas, picking runs on a throttled frame loop using the most recent cursor position.
- **Pause/disable**: The system is ON by default, and there are `pause()` / `resume()` APIs available, but automatic “turn off during heavy tasks” is not currently wired.
- **Efficiency**: Because the pick buffer is tiny (3×3) and uses simple flat colors, it runs fast and stays reliable even when draw order / transparency is involved.
...
### Render pass
- Offscreen framebuffer sized 3×3.
...
Note: If configured to use a 1×1 decision, the system still renders a 3×3 buffer and reads a single pixel from it.
...
- The implementation supports a manual global pause/resume, but does not currently auto-toggle during heavy operations.

### Edge cases and policies
- Gizmo handles are rendered in the pick pass only when the gizmo is rendered.
- Gizmo handles are rendered without depth testing so they remain pickable even when behind geometry.
