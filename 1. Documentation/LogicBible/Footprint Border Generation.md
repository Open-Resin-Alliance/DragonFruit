...
4. **Safety Margin**: Includes a configurable margin (default 2mm) beyond the actual footprint

---

## Algorithm Overview

The footprint generation uses a **three-stage pipeline**:
...
If rafts are enabled and supports exist:
...
const GRID_SIZE = 50; // 50x50 = 2,500 rays
...
|-----------|------------|-------------|----------|
| 25x25 | 625 | ~2-3ms | Good |
| **50x50** | **2,500** | **~5-8ms** | **Excellent** ⭐ |
| 75x75 | 5,625 | ~10-15ms | Outstanding |
| 100x100 | 10,000 | ~20-30ms | Overkill |
...
The convex hull is expanded outward by a configurable margin (default: 2mm).
...
- g = grid size (50)
- n = total collected points (~2,500-3,000)
...
interface RaftSettings {
  ...
  footprintBorderMargin: number;     // Margin in mm (default: 2.0)
  // ... other raft settings
}
```
...
### No Raft
- Only shows model footprint
- Raft points collection simply returns empty
